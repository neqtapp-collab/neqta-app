...
function PricingDrawer({product,settings,monthlyOverhead,selectiveCosts,directLaborHourlyCost,close,apply}:{product:Product;settings:NeqtaSettings;monthlyOverhead:number;selectiveCosts:StructureCost[];directLaborHourlyCost:number;close:()=>void;apply:(p:Product)=>void}){
  ...
  const margin=effectiveCost>0?(marginForChannel(price,effectiveCost,embeddedFees)??0):0;
  ...
  const status:ProductStatus=effectiveCost>0?statusFor(margin,target):'critical';
  ...
  </div><footer className="pricing-drawer-footer"><span>{money(product.currentPrice)} <ArrowRight/> <b>{money(price)}</b></span><button className={buttonClass('primary')} disabled={!changed||price<=0||effectiveCost<=0} onClick={()=>apply({...product,currentPrice:price,projectedMargin:margin,recommendedPrice:price>=recommended?price:recommended,status})}>Atualizar preço na NEQTA</button></footer></section></div>,document.body);
}
