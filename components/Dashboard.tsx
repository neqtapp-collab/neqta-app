...
export function Dashboard({data}:{data:DashboardOverviewResponse}){
  ...
  return <><section className="title"><h1>Visão Geral</h1><p>Saúde da sua precificação, custos e margens.</p></section>
    ...
    <section className="primary-grid">
      <article className="card insight"><h2>INSIGHT <b>NEQTA</b></h2>{priority?<div className="insight-body"><div><h3><b>{data.reviewCount} {data.reviewCount===1?'produto exige':'produtos exigem'}</b> atenção.</h3><p>{priority.name} está com <em>{priority.margin.toFixed(1).replace('.',',')}%</em> de margem projetada.</p><div className="prices"><span>{money(priority.currentPrice)}<small>Preço atual</small></span><ArrowRight/><span>{money(priority.recommendedPrice)}<small>Preço sugerido</small></span></div><Link className={buttonClass('primary')} href={routes.pricingProduct(priority.id)}>Simular ajuste<ArrowRight/></Link></div></div>:<div className="dashboard-empty"><h3>Seus produtos estão dentro das margens configuradas.</h3><p>A NEQTA continuará acompanhando custos e preços.</p></div>}</article>
      <Costs rows={data.pressuringCosts}/>
    </section>
    <section className="lower-grid">
      <article className="card actions"><h2>Próximas ações</h2>{priority?<Link href={routes.pricingProduct(priority.id)}><CheckCircle2/>Revisar preço de {priority.name}<ArrowRight/></Link>:<p className="dashboard-empty-copy">Nenhuma revisão prioritária no momento.</p>}{data.pressuringCosts.map(cost=><Link href={routes.costIngredient(cost.id)} key={cost.id}><CheckCircle2/>Revisar impacto de {cost.name}<ArrowRight/></Link>)}</article>
